...
OUTPUT_SUBDIR = "load_test_results"

SERVICES = [
    {"name": "persistence",  "lua": "teastore_persistence.lua"},
    {"name": "auth",         "lua": "teastore_auth.lua"},
    {"name": "image",        "lua": "teastore_image.lua"},
    {"name": "recommender",  "lua": "teastore_recommender.lua"},
    {"name": "webui",        "lua": "teastore_webui_isolated.lua"},
]
...
def main():
    ...
    if not log_exists:
        log_writer.writerow([
            "timestamp", "service", "intensity", "threads",
            "status", "elapsed_seconds", "output_file",
        ])

    total = len(SERVICES) * len(INTENSITIES) * len(THREAD_COUNTS)
    current = 0
    passed = 0
    failed = 0
    skipped = 0
    ...
    print("=" * 60)
    print("  TeaStore Isolated LOAD Test Runner")
            ...
            for intensity in INTENSITIES:
                for threads in THREAD_COUNTS:
                    ...
                    output_file = os.path.join(
                        OUTPUT_SUBDIR,
                        f"rq1_{service['name']}_{intensity['name']}_t{threads}.csv",
                    )
                    ...
                    if os.path.exists(abs_output) and os.path.getsize(abs_output) > 0:
                        ...
                        print(
                            f"[{current}/{total}] "
                            f"{service['name']} | {intensity['name']} | {threads} threads "
                    ...
                    print(
                        f"[{current}/{total}] "
                        f"{service['name']} | {intensity['name']} intensity | {threads} threads"
                    )
                    ...
                    status = run_test(
                        service["lua"], intensity["csv"], threads, output_file
                    )
